...
class Evidence(BaseModel):
    ...
    source: str = "baseline"  # baseline | provider_rules | llm | manual
    ...
    for q in questions:
        ...
        if value is not None:
            ...
            if t in ["currency","number"]:
                v = norm_money_local(v) if t == "currency" else (float(v) if re.fullmatch(r"\d+(?:\.\d+)?", str(v)) else None)
...
def llm_fill(pages: List[str], questions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    sys = "You extract answers from UK pension documents. Return ONLY valid JSON with exactly the requested keys and null for unknown. Do not invent data."
...
- Dates: prefer DD/MM/YYYY or as printed; do not rewrite month words if ambiguous.
...
"""
    ...
    def call_llm(chunk_text: str) -> Dict[str, Any]:
        body = {
            "model": OPENAI_MODEL,
            "temperature": 0.0,
            ...
            "messages": [
                ...
                {"role":"user","content":make_prompt(chunk_text)}
            ],
        }
    ...
    for ptxt in pages:
        ...
        if not txt:
            continue
        try:
            candidates.append(call_llm(txt))
        except Exception:
            continue
    ...
    merged: Dict[str, Any] = {}
    for k in keys:
        vals = [c.get(k) for c in candidates if c.get(k) not in [None, "", []]]
        if not vals:
            merged[k] = None
            continue
        # majority vote on stringified normalised values
        norm = lambda v: str(v).strip().lower()
        most_common = Counter([norm(v) for v in vals]).most_common(1)[0][0]
        chosen = next(v for v in vals if norm(v) == most_common)
        merged[k] = chosen
    return merged
...
def items_to_df(items: List[Answer]) -> pd.DataFrame:
    ...
    for a in items:
        rows.append({
            "Key": a.key,
            "Question": a.label,
            "Answer": "" if a.value is None else a.value,
            "Confidence": a.confidence,
            "Source": a.evidence.source if a.evidence else "",
            "Snippet": a.evidence.snippet if a.evidence else ""
        })
    return pd.DataFrame(rows)

def _normalise_by_type(val, t):
    if val is None:
        return None
    try:
        if t == "currency": return float(str(val).replace(",",""))
        if t == "number":   return float(val)
        if t == "boolean":  return str(val).strip().lower() in ["true","yes","y","1"]
        if t in ["string","date"]: return str(val).strip()
        return val
    except:
        return val
...
if uploaded:
    ...
    # 4) Merge with simple agreement logic
    items: List[Answer] = []
    for q in questions:
        key, label, t = q["key"], q["label"], q["type"]
        b = base.get(key, {})
        b_val, b_conf, b_ev = b.get("value"), b.get("confidence", 0), b.get("evidence")
        l_raw = llm_data.get(key)
        l_val = _normalise_by_type(l_raw, t)

        if b_val is not None and l_val is not None:
            if str(b_val).strip().lower() == str(l_val).strip().lower():
                final, conf, src = b_val, max(b_conf, 85), "baseline+llm"
                ev = Evidence(**b_ev) if b_ev else None
            else:
                if b_conf >= 80:
                    final, conf, src = b_val, b_conf - 5, "baseline"
                    ev = Evidence(**b_ev) if b_ev else None
                else:
                    final, conf, src = l_val, 75, "llm"
                    ev = Evidence(page=None, snippet=None, source="llm")
        elif b_val is not None:
            final, conf, src = b_val, b_conf, "baseline"
            ev = Evidence(**b_ev) if b_ev else None
        elif l_val is not None:
            final, conf, src = l_val, 72, "llm"
            ev = Evidence(page=None, snippet=None, source="llm")
        else:
            final, conf, src, ev = None, 0, "baseline", None
    ...
    edited = st.data_editor(
        ...
        disabled=["Key","Question","Source","Snippet"]
    )
